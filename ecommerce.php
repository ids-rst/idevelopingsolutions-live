<?php include 'header.php'; ?>
<!-- Breadcrumbs Start -->
<div class="rs-breadcrumbs img2">
  <div class="breadcrumbs-inner text-center">
    <h1 class="page-title">Ecommerce</h1>
    <ul>
      <li title="Home">
        <a class="active" href="index.php">Home</a>
      </li>
      <li title="Services">
        <a class="active" href="services.php">Services</a>
      </li>
      <li>Ecommerce</li>
    </ul>
  </div>
</div>
<!-- Breadcrumbs End -->
<!-- About Section Start -->
<div class="rs-about pt-120 pb-120 md-pt-80 md-pb-80">
  <div class="container">
    <div class="row align-items-center">
      <div class="col-lg-6 pr-40 md-pr-15 md-mb-50">
        <div class="sec-title4">
          <span class="sub-text">Ecommerce</span>
          <h2 class="title">Build a customized online store with high functionality and security.</h2>
          <div class="heading-line custom-line"></div>
          <div class="desc desc-big">
            Ecommerce development has become the most preferred option for business owners when it comes to setting up a new shop online.
          </div>
          <div class="desc">
            Our Experienced team can help you create a compelling, modern online store with flexible functionality, cutting edge design, and seamless user experiences. We build custom solutions for all types of businesses. Our experienced and talented team will work closely with you to create your unique digital presence using the latest eCommerce technologies.
          </div>
          <div class="btn-part mt-45">
            <a class="readon learn-more" href="contact.php">Contact Us</a>
          </div>
        </div>
      </div>
      <div class="col-lg-6">
        <div class="software-img">
          <img src="assets/images/our-services/ecommerce-development.png" alt="images">
        </div>
      </div>
    </div>
  </div>
  <!-- Services Section Start -->
  <div class="rs-services style5 pt-120 md-pt-80">
    <div class="container">
      <div class="row">
        <div class="col-lg-4 md-mb-30">
          <div class="services-item">
            <div class="services-icon">
              <img src="assets/images/our-services/1.png" alt="Images">
            </div>
            <div class="services-content">
              <h3 class="services-title">Expert Peoples</h3>
              <p class="services-desc">
                We have a network of top performers across the globe.
              </p>
            </div>
          </div>
        </div>
        <div class="col-lg-4 md-mb-30">
          <div class="services-item">
            <div class="services-icon">
              <img src="assets/images/our-services/2.png" alt="Images">
            </div>
            <div class="services-content">
              <h3 class="services-title">First Growing Process</h3>
              <p class="services-desc">
                Grow your business with the first growing process.
              </p>
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="services-item">
            <div class="services-icon">
              <img src="assets/images/our-services/3.png" alt="Images">
            </div>
            <div class="services-content">
              <h3 class="services-title">Creative Ideas</h3>
              <p class="services-desc">
                Ideas that stick in the minds of our customers.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- Services Section End -->
</div>
<!-- About Section Start -->
<div id="rs-about" class="rs-about gray-color pt-120 pb-120 md-pt-80 md-pb-60">
  <div class="container">
    <div class="row">
      <div class="col-lg-6 pr-45 md-pr-15 md-mb-50">
        <div class="choose-img">
          <img src="assets/images/our-services/watch.png" alt="images">
        </div>
      </div>
      <div class="col-lg-6">
        <div class="sec-title4 mb-30">
          <div class="sub-text choose">Why Choose Us</div>
          <h2 class="title">
            Our Competency Level
          </h2>
        </div>
        <!-- Skillbar Section Start -->
        <div class="rs-skillbar style1 modify1">
          <div class="cl-skill-bar">
            <!-- Start Skill Bar -->
            <span class="skillbar-title">Theme implementation 90</span>
            <div class="skillbar" data-percent="90">
              <p class="skillbar-bar"></p>
              <span class="skill-bar-percent"></span>
            </div>
            <!-- Start Skill Bar -->
            <span class="skillbar-title">Core development 80%</span>
            <div class="skillbar" data-percent="80">
              <p class="skillbar-bar"></p>
              <span class="skill-bar-percent"></span>
            </div>
            <!-- Start Skill Bar -->
            <span class="skillbar-title">web services development for mobile apps 95%</span>
            <div class="skillbar" data-percent="95">
              <p class="skillbar-bar"></p>
              <span class="skill-bar-percent"></span>
            </div>
          </div>
        </div>
        <!-- Skillbar Section End -->
      </div>
    </div>
  </div>
</div>
<!-- About Section End -->
<!-- Counter Section Start -->
<div class="rs-counter style3 modify1 pt-90 pb-80">
  <div class="container">
    <div class="counter-top-area">
      <div class="row">
        <div class="col-lg-4 col-md-6 md-mb-30">
          <div class="counter-list">
            <div class="counter-text">
              <div class="count-number">
                <span class="rs-count">8</span>
                <span class="prefix">+</span>
              </div>
              <h3 class="title">Years Experience</h3>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-6 md-mb-30">
          <div class="counter-list">
            <div class="counter-text">
              <div class="count-number">
                <span class="rs-count">500</span>
                <span class="prefix">+</span>
              </div>
              <h3 class="title">Website Built</h3>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-6 sm-mb-30">
          <div class="counter-list">
            <div class="counter-text">
              <div class="count-number">
                <span class="rs-count">100</span>
                <span class="prefix">+</span>
              </div>
              <h3 class="title">Happy Clients</h3>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- Counter Section End -->
</div>
<!-- Main content End -->
<?php include 'footer.php'; ?>